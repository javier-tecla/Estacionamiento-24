...
class AjusteController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (!$ajuste || !$ajuste->logo){
            $rules['logo'] = 'required|image|mimes:jpeg,png,webp,jpg,gif,svg|max:2048';
        }else{
             $rules['logo'] = 'nullable|image|mimes:jpeg,png,webp,jpg,gif,svg|max:2048';
        }

        if (!$ajuste || !$ajuste->logo_auto){
            $rules['logo_auto'] = 'required|image|mimes:jpeg,png,webp,jpg,gif,svg|max:2048';
        }else{
             $rules['logo_auto'] = 'nullable|image|mimes:jpeg,png,webp,jpg,gif,svg|max:2048';
        }
