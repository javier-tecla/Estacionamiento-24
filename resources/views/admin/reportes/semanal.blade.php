...
<body>
    <div class="container">
        <div class="header">
            <h1>Reporte de facturaciones semanal</h1>
            <p>Generado el: {{ now()->format('d/m/Y')." ".now()->format('H:i:s A') }}</p>
        </div>

        <div class="report-info">
            <p><b>Periodo del reporte:</b> {{ \Carbon\Carbon::parse($fecha_inicio)->format('d/m/Y')." al ". \Carbon\Carbon::parse($fecha_fin)->format('d/m/Y') }} </p>
        </div>

        @if ($facturaciones->isEmpty())
            <p style="text-align: center">No hay facturaciones en el periodo seleccionado, no se encontraron facturas en el rango de fechas.</p>
        @else 
            <table>
                <thead>
                    <tr>
